...
// Function: appendSvgString
// This function appends the input SVG XML to the current layer.
//
// Parameters:
// xmlString - The SVG as XML text.
//
...
this.appendSvgString = function(xmlString) {
	var j, ts;
	try {
		var batchCmd = new BatchCommand('Import Image');
        ...
        // Extract the children and append it to the canvas
        while (svg.firstElementChild) {
            var new_ele = svg.firstElementChild;
            new_ele.id = getNextId();
            (current_group || getCurrentDrawing().getCurrentLayer()).appendChild(new_ele);
            batchCmd.addSubCommand(new InsertElementCommand(new_ele));
        }
		clearSelection();
        // TODO: Messes up grouping
		//addToSelection([new_ele]);
